...
public class Level3State extends GameState{
    ...
    public void init() {
        ...
        map = new Map("/level3.map");
        pointsText = "Points: 0/0";
        winText = "";
    ...
    public void tick() {
        ...
        if(player.points == map.getCoinsAmount())
            map.getDoor().setOpen(true);
        if(player.isCollidingWithDoor && map.getDoor().getOpen())
    ...
    public void draw(Graphics g) {
        ...
        int winSize = 48;
        g.setFont(new Font("Arial", Font.PLAIN, winSize));
        g.drawString(winText, GamePanel.WIDTH/2 - (winSize / 4 * winText.length()), GamePanel.HEIGHT/2);
    }
